...
for (tissue in tissues) {
  print(tissue)
  unfiltered_db <- "/home/jennifer/new_PredDB/elastic_net/model_training/revised_scripts/ALL_unfiltered_2.db"
  filtered_db <- "/home/jennifer/new_PredDB/elastic_net/model_training/scripts/ALL_filtered_test.db"
  in_conn <- dbConnect(driver, unfiltered_db)
  out_conn <- dbConnect(driver, filtered_db)
  model_summaries <- dbGetQuery(in_conn, 'select * from extra  where zscore_pval < 0.05 and rho_avg > 0.1')
  ...
  weights <- weights %>% filter(gene %in% model_summaries$gene) %>% rename(eff_allele = eff_allele, weight = weight)
